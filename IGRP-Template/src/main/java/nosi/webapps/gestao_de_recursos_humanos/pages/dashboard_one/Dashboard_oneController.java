...
public class Dashboard_oneController extends Controller {
	public Response actionIndex() throws IOException, IllegalArgumentException, IllegalAccessException{
		Dashboard_one model = new Dashboard_one();
		model.load();
		Dashboard_oneView view = new Dashboard_oneView();
		/*----#gen-example
		  EXAMPLES COPY/PASTE:
	...
	if(Core.isNotNullOrZero(model.getDepartamento_id())){
		temployeesfilter.orWhere("departmentId.departmentId","=",Core.toInt(model.getDepartamento_id()));
	}
	if(Core.isNotNullOrZero(model.getFuncao_id())){
		temployeesfilter.orWhere("jobId.jobId","=",Core.toInt(model.getFuncao_id()));
	}
	List<TEmployees> temployeesList = temployeesfilter.orderByAsc("hireDate").all();
	List<Dashboard_one.Timeline_1> temployeesTable = new ArrayList<>();
	if(Core.isNotNull(temployeesList)){
		for(TEmployees temployees : temployeesList){
			Dashboard_one.Timeline_1 row  = new Dashboard_one.Timeline_1();
			...
			row.setTimeline_1_btn(Core.getIGRPLink("gestao_de_recursos_humanos", "Funcionario", "index"+"&p_id=" +temployees.getEmployeeId()));
			row.setTimeline_1_btn_desc(temployees.getFirstName());
	...
	return this.forward("gestao_de_recursos_humanos","Dashboard_one","index",this.queryString());/* End-Code-Block  */
		/*----#start-code(filtrar)----*/
